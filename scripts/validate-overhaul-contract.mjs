...
const householdPrincipalsRegressionPath = join(root, 'supabase/tests/atlas_scoped_household_principals_test.sql')
const householdPrincipalsRegression = existsSync(householdPrincipalsRegressionPath) ? readFileSync(householdPrincipalsRegressionPath, 'utf8') : ''
const householdPrincipalsMigrationPath = join(root, 'migrations/20260907071500_atlas_scoped_household_principals.sql')
const householdPrincipalsMigration = existsSync(householdPrincipalsMigrationPath) ? readFileSync(householdPrincipalsMigrationPath, 'utf8') : ''
...
requireContract(/nicole-codex/.test(workerText) && /nicole-codex/.test(appText), 'fixed scoped household principals must be represented')
requireContract(!/["'`]\/shared(?:[\/"'`]|$)/.test(workerText + appText), 'household support must not add a separate shared page or API')
requireContract(/ATLAS_OWNER_EMAILS/.test(workerText), 'Worker must enforce an owner email allowlist')
requireContract(/ATLAS_HUMAN_ACCESS_JSON/.test(workerText) && /human_access/.test(workerText), 'verified household human access must use a distinct scoped identity mapping')
...
requireContract(/select plan\(40\)/i.test(householdPrincipalsRegression) && /select \* from finish\(\)/i.test(householdPrincipalsRegression), '40-assertion scoped household-principal pgTAP regression contract is required')
requireContract(householdPrincipalsMigration.indexOf('create or replace function public.atlas_validate_principal_roster()') < householdPrincipalsMigration.indexOf('insert into public.atlas_members'), 'household roster validator must be replaced before reactivating the existing Nicole row')
requireContract(/grant execute on function public\.atlas_principal_is_active\(text\) to service_role/i.test(householdPrincipalsMigration), 'service role must execute the principal helper used by invoker triggers')
requireContract(/name = case when[\s\S]*role = case when/i.test(householdPrincipalsMigration), 'Nicole reactivation must preserve existing nonblank profile fields')
...
console.log('ATLAS owner-controlled security and architecture contract passed.')
