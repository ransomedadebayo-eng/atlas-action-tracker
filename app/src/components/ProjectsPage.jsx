...
function ProjectForm({ selectedBusiness, members, currentMemberId, onCancel, onCreated }) {
  const { BUSINESS_LIST } = useBusinessContext();
  const mutation = useCreateProject();
  const [form, setForm] = useState({
    name: '', summary: '', business: selectedBusiness || '', status: 'planned', priority: '',
    lead_id: currentMemberId, members: [currentMemberId], start_date: '', target_date: '', update_frequency: 'weekly',
  });
  ...
  function applyTemplate(template) {
    ...
    setForm(current => ({
      ...
      priority: blueprint.priority || '', lead_id: blueprint.lead_id || currentMemberId,
      members: Array.isArray(blueprint.members) ? blueprint.members : [currentMemberId],
...
function ProjectPortfolio({ selectedBusiness, searchQuery, onOpenProject }) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5">
      ...
      {creating && (currentMember ? <ProjectForm selectedBusiness={selectedBusiness} members={members} currentMemberId={currentMember.id} onCancel={() => setCreating(false)} onCreated={project => onOpenProject(project.id)} /> : currentMemberQuery.isError ? <div className="card flex flex-wrap items-center gap-3 p-4" role="alert"><p className="min-w-0 flex-1 text-sm text-red-400">{currentMemberQuery.error?.message || 'Atlas could not verify the current principal.'}</p><button type="button" className="btn-secondary min-h-11" onClick={() => currentMemberQuery.refetch?.()}>Retry</button><button type="button" className="btn-ghost min-h-11" onClick={() => setCreating(false)}>Cancel</button></div> : <div className="card flex items-center justify-between gap-3 p-4" aria-label="Loading current principal"><p className="text-sm text-text-muted">Verifying your Atlas identity…</p><button type="button" className="btn-ghost min-h-11" onClick={() => setCreating(false)}>Cancel</button></div>)}
