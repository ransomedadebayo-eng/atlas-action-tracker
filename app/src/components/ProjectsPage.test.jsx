...
describe('ProjectsPage', () => {
  ...
  it('waits for the verified current principal before choosing project defaults', () => {
    memberHooks.useCurrentMember.mockReturnValue({ data: undefined, isError: false });
    const props = { projectId: null, selectedBusiness: null, searchQuery: '', onOpenProject: vi.fn(), onBack: vi.fn(), onSelectAction: vi.fn() };
    const view = render(<ProjectsPage {...props} />);
    fireEvent.click(screen.getByRole('button', { name: /new project/i }));
    expect(screen.getByLabelText('Loading current principal')).toBeTruthy();

    memberHooks.useCurrentMember.mockReturnValue({ data: { id: 'nicole' } });
    view.rerender(<ProjectsPage {...props} />);
    expect(screen.getByLabelText('Lead').value).toBe('nicole');
  });

  it('offers retry and cancel when current-principal lookup fails', () => {
    const refetch = vi.fn();
