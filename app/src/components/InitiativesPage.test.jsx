...
vi.mock('../hooks/useMembers.js', () => ({ useMembers: vi.fn(), useCurrentMember: vi.fn(() => ({ data: { id: 'ransomed' } })) }));
...
describe('InitiativesPage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    memberHooks.useMembers.mockReturnValue({ data: [{ id: 'ransomed', name: 'Ransomed' }, { id: 'nicole', name: 'Nicole' }, { id: 'codex', name: 'Codex' }] });
    memberHooks.useCurrentMember.mockReturnValue({ data: { id: 'ransomed' } });
  ...
  it('preserves an intentional owner choice after the default identity is applied', () => {
    memberHooks.useCurrentMember.mockReturnValue({ data: { id: 'nicole' }, isError: false });
    const props = { initiativeId: null, selectedBusiness: null, searchQuery: '', onOpenInitiative: vi.fn(), onBack: vi.fn(), onOpenProject: vi.fn() };
    const view = render(<InitiativesPage {...props} />);
    fireEvent.click(screen.getByRole('button', { name: /new initiative/i }));
    const owner = screen.getByLabelText('Owner');
    expect(owner.value).toBe('nicole');
    fireEvent.change(owner, { target: { value: 'codex' } });
    fireEvent.change(owner, { target: { value: 'ransomed' } });

    view.rerender(<InitiativesPage {...props} />);
    expect(screen.getByLabelText('Owner').value).toBe('ransomed');
  });

  it('does not render a default owner or submit while identity is pending', () => {
    const create = vi.fn();
    initiativeHooks.useCreateInitiative.mockReturnValue({ mutateAsync: create, isPending: false });
    memberHooks.useCurrentMember.mockReturnValue({ data: undefined, isError: false });
    render(<InitiativesPage initiativeId={null} selectedBusiness={null} searchQuery="" onOpenInitiative={vi.fn()} onBack={vi.fn()} onOpenProject={vi.fn()} />);
    fireEvent.click(screen.getByRole('button', { name: /new initiative/i }));
    expect(screen.getByLabelText('Loading initiative principal')).toBeTruthy();
