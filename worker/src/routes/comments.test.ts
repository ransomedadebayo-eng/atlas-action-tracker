...
describe('comment validation and aggregation',()=>{
  it('accepts HTTPS attachment metadata and revision-bound anchors',()=>{
    ...
    expect(validateCommentBody({body:'Review @nicole-codex',mentions:['nicole-codex'],attachments:[{title:'Design',url:'https://example.com'}],anchor:{field:'content',quote:'text'}})).toEqual([]);
  });
