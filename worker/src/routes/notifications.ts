...
function errorResponse(c: any, error: any, fallback: string) {
  ...
  if (error?.code === '42501' || message.includes('PRINCIPAL_REQUIRED')) return apiError(c,403,'NOTIFICATION_PRINCIPAL_REQUIRED','A principal can change only its own notification state and preferences.');
