...
import { isActivePrincipal } from '../utils/principals';
...
export class DocumentRoom extends DurableObject<Env> {
  async fetch(request: Request): Promise<Response> {
    ...
    if (!documentId || !isActivePrincipal(actor) || !clientId || clientId.length > 128) return new Response('Invalid room identity', { status: 400 });
