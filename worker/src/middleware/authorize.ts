...
function ownerOnlyRequest(path: string, method: string): boolean {
  ...
  if (/^\/api\/media\/items\/[^/]+\/status$/.test(path)) return true;
  if (/^\/api\/media\/items\/[^/]+\/feedback$/.test(path)) return true;
  if (/^\/api\/media\/daily\/\d{4}-\d{2}-\d{2}\/twitter\/start$/.test(path)) return true;
  if (path === '/api/media/sync-protocols/activate') return true;
  ...
  if (path.startsWith('/api/integrations') && method !== 'GET') return true;
  return false;
}
...
export function requiredScopeForRequest(path: string, method: string): AtlasScope | null {
  ...
  if (path.startsWith('/api/media')) return method === 'GET' ? 'media:read' : 'media:write';
