...
describe('machine principal scope routing', () => {
  it('separates read, write, completion, and assignment scopes', () => {
    expect(requiredScopeForRequest('/api/today?date=2026-09-07', 'GET')).toBe('actions:read');
  ...
  it('routes weekly plan reads, drafts, review requests, and owner publication', () => {
    ...
    expect(requiredScopeForRequest('/api/media/current', 'GET')).toBe('media:read');
    expect(requiredScopeForRequest('/api/media/plans/upsert', 'POST')).toBe('media:write');
    expect(requiredScopeForRequest('/api/media/daily/2026-08-29/twitter/start', 'POST')).toBe('media:write');
    expect(requiredScopeForRequest('/api/media/sync-manifests', 'POST')).toBe('media:write');
    expect(requiredScopeForRequest('/api/media/sync-applies', 'POST')).toBe('media:write');
    expect(requiredScopeForRequest('/api/media/input-reviews', 'POST')).toBe('media:write');
  });
